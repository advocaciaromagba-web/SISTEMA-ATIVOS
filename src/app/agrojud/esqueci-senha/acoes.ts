...
export async function pedirRedefinicaoSenha(
  ...
  if (usuario && usuario.ativo) {
    ...
    await prisma.agroUsuario.update({
      where: { id: usuario.id },
      data: { resetTokenHash: hash, resetTokenExpiraEm: expiraEm },
    });

    const link = `${marca.site}/agrojud/redefinir-senha?token=${token}`;
    try {
      await enviarEmailRedefinicaoSenha({ destinatario: email, nomeSolucao: "Agrojud", link });
    } catch (err: any) {
      console.error("[email] Falha ao enviar redefinição de senha (agrojud):", err.message);
