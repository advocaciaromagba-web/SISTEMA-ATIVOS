...
import { criarContaAgro, type ResultadoCadastro } from "./acoes";
import { DIAS_DE_TESTE, CONSULTAS_GRATIS_TESTE } from "@/lib/planos";

const inicial: ResultadoCadastro = {};

const FORMAS_COBRANCA = [
  { valor: "ASSINATURA", nome: "Assinatura", descricao: "Mensalidade fixa, com análises incluídas conforme o plano." },
  { valor: "CREDITO", nome: "Crédito pré-pago", descricao: "Carrega um saldo e cada análise desconta do saldo, sem mensalidade." },
  { valor: "AVULSO", nome: "Avulso", descricao: "Paga só quando usa, sem saldo nem mensalidade." },
];

export function FormularioCadastroAgro() {
  const [estado, acao] = useActionState(criarContaAgro, inicial);
  ...
  return (
    <form action={acao} className="cartao space-y-5">
        ...
        <div className="flex gap-2">
          {(["PJ", "PF"] as const).map((t) => (
            ...
            >
              ...
              {t === "PJ" ? "Escritório" : "Advogado autônomo"}
            </label>
          ))}
        </div>
      </div>
      ...
      <div>
        ...
        <p className="ajuda">
          {DIAS_DE_TESTE} dias de teste com {CONSULTAS_GRATIS_TESTE} análises grátis, em qualquer forma escolhida.
        </p>
      </div>
