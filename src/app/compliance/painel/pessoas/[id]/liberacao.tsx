...
import { liberarPessoaDiligencia, rebloquearPessoaDiligencia, type ResultadoAcao } from "../acoes";
 ...
 */
export function LiberacaoPessoa({
  diligenciaPessoaId,
  bloqueada,
  ehDono,
  liberacao,
  ...
  const [estado, acao] = useActionState(liberarPessoaDiligencia, inicial);

  function rebloquear() {
    setErro("");
    iniciar(async () => {
      const r = await rebloquearPessoaDiligencia(diligenciaPessoaId);
  ...
  if (bloqueada) {
    return (
      <div className="aviso-erro mt-3">
        <strong className="block">Pessoa bloqueada — restrição encontrada na auditoria</strong>
        <span className="mt-1 block">
          Enquanto bloqueada, esta pessoa fica marcada para revisão manual antes de qualquer nova verificação.
        </span>
        {ehDono && !abrir && (
        ...
        {!ehDono && (
          <span className="mt-2 block text-xs">
            Somente o responsável pela conta pode liberar uma pessoa bloqueada.
          </span>
        )}

        {abrir && ehDono && (
          <form action={acao} className="mt-3 rounded-lg border border-red-200 bg-red-50 p-4">
            <input type="hidden" name="diligenciaPessoaId" value={diligenciaPessoaId} />
            <label className="rotulo" htmlFor="justificativa">
              Por que você está liberando esta pessoa?
            </label>
            <textarea
              id="justificativa"
              name="justificativa"
  ...
  if (liberacao) {
    return (
      <div className="aviso-atencao mt-3">
        <strong className="block">Pessoa liberada manualmente</strong>
