...
export async function reanalisarProcesso(id: string): Promise<ResultadoAcao> {
  ...
  if (await testeEsgotado(conta.id, conta.statusAssinatura)) {
    return {
      erro: `Seu teste grátis já usou as ${(await configuracaoDaSolucao("COMPLIANCE_EMPRESA")).consultasGratisTeste} consultas incluídas (empresas, pessoas e processos somados). Assine um plano para continuar.`,
    };
  }
