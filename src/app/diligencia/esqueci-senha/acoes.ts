...
export async function pedirRedefinicaoSenha(
  ...
  if (usuario && usuario.ativo) {
    ...
    const link = `${marca.site}/diligencia/redefinir-senha?token=${token}`;
    try {
      await enviarEmailRedefinicaoSenha({ destinatario: email, nomeSolucao: "Due Diligence de Pessoas", link });
    } catch (err: any) {
      console.error("[email] Falha ao enviar redefinição de senha (diligencia):", err.message);
