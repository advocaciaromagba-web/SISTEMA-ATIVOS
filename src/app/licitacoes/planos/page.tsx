...
export default async function PlanosLicitacoes() {
  const planos = await planosDaSolucao("LICITACOES");
  const { diasDeTeste, consultasGratisTeste } = await configuracaoDaSolucao("LICITACOES");

  return (
    <main className="min-h-screen bg-slate-50">
      <header className="border-b border-slate-200 bg-white px-4 py-6">
        <div className="mx-auto flex max-w-5xl items-center justify-between">
          <Link href="/licitacoes/entrar" aria-label={marca.nome}>
            <MarcaLogo altura={30} prioridade />
          </Link>
          <Link href="/licitacoes/entrar" className="text-sm text-slate-500 hover:underline">
            Já tem conta? Entrar
          </Link>
        </div>
      </header>
      ...
      <div className="mx-auto max-w-5xl px-4 py-16">
        <p className="sobretitulo">Análise de licitações</p>
