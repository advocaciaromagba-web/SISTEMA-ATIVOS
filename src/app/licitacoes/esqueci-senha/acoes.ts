...
export async function pedirRedefinicaoSenha(
  ...
  if (usuario && usuario.ativo) {
    ...
    const link = `${marca.site}/licitacoes/redefinir-senha?token=${token}`;
    try {
      await enviarEmailRedefinicaoSenha({ destinatario: email, nomeSolucao: "Análise de Licitações", link });
    } catch (err: any) {
      console.error("[email] Falha ao enviar redefinição de senha (licitacoes):", err.message);
