...
export async function pedirRedefinicaoSenha(
  ...
  const usuario = await prisma.serasaUsuario.findUnique({ where: { email } });

  if (usuario && usuario.ativo) {
    const { token, hash, expiraEm } = gerarTokenRedefinicao();
    await prisma.serasaUsuario.update({
      where: { id: usuario.id },
      data: { resetTokenHash: hash, resetTokenExpiraEm: expiraEm },
    });

    const link = `${marca.site}/serasa/redefinir-senha?token=${token}`;
    try {
      await enviarEmailRedefinicaoSenha({ destinatario: email, nomeSolucao: "Consulta Cadastral", link });
    } catch (err: any) {
      console.error("[email] Falha ao enviar redefinição de senha (serasa):", err.message);
