...
export async function pedirRedefinicaoSenha(
  ...
  if (usuario && usuario.ativo) {
    ...
    const link = `${marca.site}/redefinir-senha?token=${token}`;
    try {
      await enviarEmailRedefinicaoSenha({ destinatario: email, nomeSolucao: "Gestão de Ativos e Operações", link });
    } catch (err: any) {
      console.error("[email] Falha ao enviar redefinição de senha (ativos):", err.message);
