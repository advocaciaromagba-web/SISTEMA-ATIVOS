...
export async function salvarLicitante(_anterior: ResultadoAcao, dados: FormData): Promise<ResultadoAcao> {
  ...
  if (id) {
    const existente = await prisma.licitanteEmpresa.findFirst({ where: { id, organizacaoId: organizacao.id } });
    ...
    await registrar({
      ...
      entidade: "LicitanteEmpresa",
      entidadeId: id,
      detalhe: { nome },
    });
  } else {
    const criada = await prisma.licitanteEmpresa.create({ data: { ...valores, organizacaoId: organizacao.id } });
  ...
  redirect(`/painel/licitacoes/licitantes/${licitanteId}`);
}
...
export async function excluirLicitante(id: string): Promise<ResultadoAcao> {
  ...
  await registrar({
    ...
    entidade: "LicitanteEmpresa",
    entidadeId: id,
    detalhe: { nome: licitante.nome },
  });

  revalidatePath("/painel/licitacoes/licitantes");
  redirect("/painel/licitacoes/licitantes");
}
...
export async function anexarDocumentoPessoal(_anterior: ResultadoAcao, dados: FormData): Promise<ResultadoAcao> {
  ...
  const licitante = await prisma.licitanteEmpresa.findFirst({
    where: { id: licitanteEmpresaId, organizacaoId: organizacao.id },
  });
  if (!licitante) return { erro: "Empresa licitante não encontrada." };
  ...
  revalidatePath(`/painel/licitacoes/licitantes/${licitanteEmpresaId}`);
  return { ok: true };
}
...
export async function excluirDocumentoPessoal(id: string, licitanteEmpresaId: string): Promise<ResultadoAcao> {
  ...
  const doc = await prisma.licitanteDocumentoPessoal.findFirst({ where: { id, organizacaoId: organizacao.id } });
...
export async function gerarEnvelope(_anterior: ResultadoAcao, dados: FormData): Promise<ResultadoAcao> {
  ...
  const [licitante, edital] = await Promise.all([
    prisma.licitanteEmpresa.findFirst({ where: { id: licitanteEmpresaId, organizacaoId: organizacao.id } }),
    prisma.editalInteresse.findFirst({ where: { id: editalInteresseId, organizacaoId: organizacao.id } }),
  ]);
  if (!licitante) return { erro: "Empresa licitante não encontrada." };
  ...
  await registrar({
    ...
    detalhe: { licitante: licitante.nome, certame: `${edital.modalidade} ${edital.numeroCertame}` },
  });

  revalidatePath(`/painel/licitacoes/licitantes/${licitanteEmpresaId}`);
  redirect(`/painel/licitacoes/licitantes/${licitanteEmpresaId}`);
}
