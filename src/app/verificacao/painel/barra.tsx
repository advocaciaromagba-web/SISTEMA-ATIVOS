...
export function BarraVerificacao({ marcaNome, contaNome, usuarioNome }: { marcaNome: string; contaNome: string; usuarioNome: string }) {
  return (
    <header className="faixa-escura border-b-2 border-[color:var(--marca-destaque)]">
      <div className="mx-auto flex max-w-7xl flex-wrap items-center gap-x-6 gap-y-3 px-4 py-3">
        <Link href="/verificacao/painel" aria-label={marcaNome}>
          <MarcaEscura nome={marcaNome} assinatura="Verificação de Documentos" />
        </Link>
        ...
        <div className="flex items-center gap-3 text-sm">
          <Link href="/verificacao/painel/seguranca" className="text-white/70 hover:text-white hover:underline">
            Segurança
          </Link>
          ...
          <button
            onClick={() => signOut({ callbackUrl: "/verificacao/entrar" })}
