...
import { criarContaVerificacao, type ResultadoCadastro } from "./acoes";
import { DIAS_DE_TESTE, CONSULTAS_GRATIS_TESTE } from "@/lib/planos";

const inicial: ResultadoCadastro = {};

const FORMAS_COBRANCA = [
  { valor: "ASSINATURA", nome: "Assinatura", descricao: "Mensalidade fixa, com verificações incluídas conforme o plano." },
  { valor: "CREDITO", nome: "Crédito pré-pago", descricao: "Carrega um saldo e cada documento desconta do saldo, sem mensalidade." },
  { valor: "AVULSO", nome: "Avulso", descricao: "Paga só quando usa, sem saldo nem mensalidade." },
];

export function FormularioCadastroVerificacao() {
  const [estado, acao] = useActionState(criarContaVerificacao, inicial);
  ...
  return (
    <form action={acao} className="cartao space-y-5">
      ...
      <div>
        ...
        <p className="ajuda">
          {DIAS_DE_TESTE} dias de teste com {CONSULTAS_GRATIS_TESTE} documentos grátis, em qualquer forma escolhida.
        </p>
      </div>
