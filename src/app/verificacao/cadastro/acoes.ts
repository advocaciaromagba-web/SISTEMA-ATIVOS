...
export async function criarContaVerificacao(_anterior: ResultadoCadastro, dados: FormData): Promise<ResultadoCadastro> {
  ...
  const { diasDeTeste } = await configuracaoDaSolucao("VERIFICACAO_DOCUMENTOS");
  const testeExpiraEm = new Date(Date.now() + diasDeTeste * 24 * 60 * 60 * 1000);

  const contaCriada = await prisma.verificacaoConta.create({
    data: {
      tipo,
      nome,
  ...
  await registrarAceite({
    solucao: "VERIFICACAO_DOCUMENTOS",
