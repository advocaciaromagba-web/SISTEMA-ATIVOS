...
async function bloqueado(email: string): Promise<number> {
  ...
  const tentativas = await prisma.agroTentativaLogin.count({ where: { email, criadoEm: { gte: desde } } });
  if (tentativas < MAX_TENTATIVAS) return 0;
  return JANELA_MINUTOS;
}

export const authOptionsAgro: NextAuthOptions = {
  session: { strategy: "jwt", maxAge: 8 * 60 * 60 },
  pages: { signIn: "/agrojud/entrar" },
  cookies: {
    sessionToken: {
      name: "agro.session-token",
  ...
  providers: [
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        const interno = await autorizarAdminNaSolucao("AGROJUD", credentials);
        if (interno) {
          await prisma.agroTentativaLogin.deleteMany({ where: { email } });
          return {
            id: interno.id,
            name: interno.nome,
            email: interno.email,
            agroContaId: interno.contaId,
            papel: interno.papel,
          };
        }
        ...
        if (!usuario || !usuario.ativo || !senhaConfere) {
          await prisma.agroTentativaLogin.create({ data: { email } });
          return null;
        }

        if (!usuario.agroConta.ativa) {
        ...
        if (usuario.agroConta.bloqueadoEm) {
        ...
        await prisma.agroTentativaLogin.deleteMany({ where: { email } });
        await prisma.agroUsuario.update({ where: { id: usuario.id }, data: { ultimoAcesso: new Date() } });

        return {
          id: usuario.id,
          name: usuario.nome,
          email: usuario.email,
          agroContaId: usuario.agroContaId,
          papel: usuario.papel,
        };
      },
    }),
